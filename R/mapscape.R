...
#;        aggregate and donut views of each sample. 
#'
#'   }
#'
...
mapscape <- function(clonal_prev, 
                     ...
                     static_tree_title = "Cluster_Tree",
  ...
  if (missing(static_tree_image)) {
    static_tree_image = img_ref
  }
  ...
  # check static tree image is png
  if (!grepl('.png', static_tree_image)) {
    stop("The static tree image must be in PNG format (must end in \".png\").")
  }
  
  static_tree_img_base64 <- base64enc::base64encode(static_tree_image)
  ...
  x = list(
    ...
    static_tree_title = as.character(static_tree_title),
    classification_title = as.character(classification_title),
    img_ref = img_ref_base64,
    static_tree_image = static_tree_img_base64,
